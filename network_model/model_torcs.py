...
class ModelTorcs(Model):
    def build_network(self, name, network, trainable, action_type_no):
        ...
        with tf.variable_scope(name):
            ...
            W_actor_conv2, b_actor_conv2 = self.make_layer_variables([3, 3, 16, 32], trainable, "actor_conv2")
            ...
            conv_out_size = np.prod(h_actor_conv2._shape[1:]).value
    
            h_actor_conv2_flat = tf.reshape(h_actor_conv2, [-1, conv_out_size], name="h_actor_conv2_flat")
            self.print_log(h_actor_conv2_flat)
    
            # Fourth layer is fully connected with 256 relu units
            W_actor_fc1, b_actor_fc1 = self.make_layer_variables([conv_out_size, 256], trainable, "actor_fc1")
    
            h_actor_fc1 = tf.nn.relu(tf.matmul(h_actor_conv2_flat, W_actor_fc1) + b_actor_fc1, name="h_actor_fc1")
            self.print_log(h_actor_fc1)
            
            W_steering, b_steering = self.make_layer_variables([256, 1], trainable, "steering")    
            h_steering = tf.tanh(tf.matmul(h_actor_fc1, W_steering) + b_steering, name="h_steering")
            self.print_log(h_steering)
            
            W_acc, b_acc = self.make_layer_variables([256, 1], trainable, "acc")    
            h_acc = tf.sigmoid(tf.matmul(h_actor_fc1, W_acc) + b_acc, name="h_acc")
            self.print_log(h_acc)
            
            W_brake, b_brake = self.make_layer_variables([256, 1], trainable, "brake")    
            ...
            #actor_y = tf.reshape(tf.concat(1, [h_steering, h_acc, h_brake]), [-1, action_type_no])
            ...
            # Third layer convolves 32 3x3 filters with stride 2 with relu
            W_critic_conv2, b_critic_conv2 = self.make_layer_variables([3, 3, 16, 32], trainable, "critic_conv2")
            ...
            conv_out_size = np.prod(self.h_critic_conv2._shape[1:]).value
    
            h_critic_conv2_flat = tf.reshape(self.h_critic_conv2, [-1, conv_out_size], name="h_critic_conv2_flat")
            self.print_log(h_critic_conv2_flat)
    
            self.h_critic_concat = tf.concat(1, [h_critic_conv2_flat, action_in])
    
            # Fourth layer is fully connected with 256 relu units
            W_critic_fc1, b_critic_fc1 = self.make_layer_variables([conv_out_size + action_type_no, 256], trainable, "critic_fc1")
            ...
            W_critic_fc2, b_critic_fc2 = self.make_layer_variables([256, self.action_type_no], trainable, "critic_fc4")    
            #W_critic_fc2, b_critic_fc2 = self.make_layer_variables([256, 1], trainable, "critic_fc4")
